...
int ds_cgroup_host_create(struct ds_config *cfg) {
  ...
  for (int i = 0; i < n; i++) {
    ...
    /* For Cgroup V2, enable available controllers in the parent group so they
     * are available in the container group. */
    if (hosts[i].version == 2) {
      char controllers_path[PATH_MAX];
      char available[1024] = {0};
      safe_strncpy(controllers_path, hosts[i].mountpoint,
                   sizeof(controllers_path));
      strncat(controllers_path, "/cgroup.controllers",
              sizeof(controllers_path) - strlen(controllers_path) - 1);

      if (read_file(controllers_path, available, sizeof(available)) > 0) {
        char to_enable[1024] = {0};
        char *saveptr;
        char *token = strtok_r(available, " ", &saveptr);
        while (token) {
          strncat(to_enable, "+", sizeof(to_enable) - strlen(to_enable) - 1);
          strncat(to_enable, token, sizeof(to_enable) - strlen(to_enable) - 1);
          strncat(to_enable, " ", sizeof(to_enable) - strlen(to_enable) - 1);
        ...
        if (to_enable[0]) {
          ...
          (void)write_file(subtree_ctrl, to_enable);
        }
      }
    }
    ...
    if (hosts[i].version == 2) {
      ...
      if (cfg->memory_limit > 0) {
        if (ds_cgroup_match_token(available, " \n\t", "memory")) {
          ...
          ds_warn("[CGROUP] Memory controller not supported by kernel.");
        }
      }
      if (cfg->cpu_quota > 0) {
        if (ds_cgroup_match_token(available, " \n\t", "cpu")) {
          ...
          ds_warn("[CGROUP] CPU controller not supported by kernel.");
        }
      }
      if (cfg->pids_limit > 0) {
        if (ds_cgroup_match_token(available, " \n\t", "pids")) {
          ...
          ds_warn("[CGROUP] PIDs controller not supported by kernel.");
        }
      }
    } else {
      /* Cgroup V1 */
      if (cfg->memory_limit > 0) {
        if (ds_cgroup_match_controller_v1(hosts[i].controllers, "memory")) {
          ...
          ds_warn("[CGROUP] Memory controller (V1) not supported by kernel.");
        }
      }
      if (cfg->cpu_quota > 0) {
        if (ds_cgroup_match_controller_v1(hosts[i].controllers, "cpu") ||
          ...
          if (write_file(file_path, val) < 0) {
            ds_warn("[CGROUP] Failed to set CPU limit (V1): %s",
                    strerror(errno));
            errors++;
          }
        } else {
          ds_warn("[CGROUP] CPU controller (V1) not supported by kernel.");
        }
      }
      if (cfg->pids_limit > 0) {
        if (ds_cgroup_match_controller_v1(hosts[i].controllers, "pids")) {
          ...
          snprintf(val, sizeof(val), "%lld", cfg->pids_limit);
          if (write_file(file_path, val) < 0) {
            ds_warn("[CGROUP] Failed to set PIDs limit (V1): %s",
                    strerror(errno));
            errors++;
          }
        } else {
          ds_warn("[CGROUP] PIDs controller (V1) not supported by kernel.");
        }
      }
    }
  }
